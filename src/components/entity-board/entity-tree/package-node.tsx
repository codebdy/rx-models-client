...
export const PackageNode = observer(
  (props: { key?: string; packageStore: PackageStore }) => {
    const { packageStore } = props;
    const appStore = useAppStore();
    const rootStore = useEntityBoardStore();
    const [excutePublish, { loading, error }] = useLazyAxios(
      API_PUBLISH_PACKAGE,
      {
        onCompleted() {
    ...
    const handelExportInterface = () => {
      if (rootStore.changed) {
        ...
        const packageGenerator = new PackageSourceGenerator(
          packageStore,
          rootStore
        );
