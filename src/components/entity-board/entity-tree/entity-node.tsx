    ...
    <TreeItem
      ...
      label={
        ...
        >
          ...
          <NodeText>
            <div style={{ marginLeft: "-8px" }}>{entityStore.name}</div>
          </NodeText>
        </TreeNodeLabel>
      }
    >
      {entityStore.columns?.length > 0 && (
        <TreeItem
          nodeId={entityStore.uuid + "columns"}
          label={
            <TreeNodeLabel
              action={
        ...
        >
          {entityStore.columns.map((column) => {
            return <ColumnNode key={column.uuid} columnStore={column} />;
          })}
        </TreeItem>
      )}
